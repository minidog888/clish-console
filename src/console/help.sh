...
_clish_console_console_help_list_commands() {
    ...
    # Print ungrouped commands (command names in GREEN)
    for cmd in "${ungrouped[@]}"; do
        local desc
        desc="$(_get_cmd_description "$cmd")"
        printf "    ${_GREEN}%-24s${_RESET} %s\n" "$cmd" "$desc"
    done
    [[ ${#ungrouped[@]} -gt 0 ]] && echo
    ...
    # Print group names in YELLOW, commands in GREEN
...
/_/  /_/\____/_____/\____/_____/_/  |_/____/_/ /_/"
    ...
    # Project name (green) and version (yellow)
    if [[ -n "$project_name" ]]; then
        echo -e "${_GREEN}${project_name}${_RESET} ${_YELLOW}${version}${_RESET} (Generated by Modulash Clish Console)"
    else
        echo -e "${_GREEN}Modulash${_RESET} ${_YELLOW}${version}${_RESET} (Generated by Modulash Clish Console)"
    fi

    # Description (green)
    if [[ -n "$desc" ]]; then
        echo -e "  ${_GREEN}${desc}${_RESET}"
    fi
    echo
